...
const String NEWS_JSON_URL = 'https://your-gh-pages.github.io/data/news.json';
const String SUBJECTS_JSON_URL = 'https://your-gh-pages.github.io/data/subjects.json';
/// ------------------------------------
...
class NewsItem {
  ...
  NewsItem({required this.id, required this.title, required this.summary, required this.url});
  factory NewsItem.fromJson(Map<String, dynamic> j) =>
      NewsItem(id: j['id'] ?? '', title: j['title'] ?? '', summary: j['summary'] ?? '', url: j['url'] ?? '');
}
...
class Subject {
  ...
  factory Subject.fromJson(Map<String, dynamic> j) => Subject(
      id: j['id'] ?? '', name: j['name'] ?? '', quizzesUrl: j['quizzes_url'] ?? j['quizzesUrl'] ?? '');
}

class QuizOverview {
  final String id, title, description, questionsUrl;
  QuizOverview({required this.id, required this.title, required this.description, required this.questionsUrl});
...
class TFQuestion {
  ...
  factory TFQuestion.fromJson(Map<String, dynamic> j) =>
      TFQuestion(id: j['id'] ?? '', text: j['text'] ?? '', answer: j['answer'] == true);
}
...
class WebLocalStorage implements AttemptStorage {
  ...
  Future<List<QuizAttempt>> getAttempts() async {
    ...
    return arr.map((e) => QuizAttempt.fromJson(Map<String, dynamic>.from(e))).toList();
  }
...
class NewsPage extends StatelessWidget {
  Future<List<NewsItem>> fetchNews() async {
    ...
    if (data == null) return [];
    return (data as List).map((e) => NewsItem.fromJson(Map<String, dynamic>.from(e))).toList();
  }
...
class SubjectsPage extends StatelessWidget {
  Future<List<Subject>> fetchSubjects() async {
    ...
    if (data == null) return [];
    return (data as List).map((e) => Subject.fromJson(Map<String, dynamic>.from(e))).toList();
  }
          ...
          itemBuilder: (context, i) {
            ...
            return Card(
              child: ListTile(
                ...
                onTap: () {
                  Navigator.of(context).push(MaterialPageRoute(builder: (_) => QuizzesPage(subject: sub)));
                },
              ),
            );
          },
...
class QuizzesPage extends StatelessWidget {
  ...
  Future<List<QuizOverview>> fetchQuizzes() async {
    ...
    if (data == null) return [];
    return (data as List).map((e) => QuizOverview.fromJson(Map<String, dynamic>.from(e))).toList();
  }
            ...
            itemBuilder: (context, i) {
              ...
              return Card(
                child: ListTile(
                  ...
                  onTap: () {
                    Navigator.of(context).push(MaterialPageRoute(builder: (_) => QuizPage(quiz: q)));
                  },
                ),
              );
            },
...
class _QuizPageState extends State<QuizPage> {
  ...
  Future<List<TFQuestion>> fetchQuestions() async {
    ...
    if (data == null) return [];
    final qs = (data['questions'] as List).map((e) => TFQuestion.fromJson(Map<String, dynamic>.from(e))).toList();
    return qs;
  }
  ...
  void _submit(List<TFQuestion> qs) async {
    ...
    for (int i = 0; i < qs.length; i++) {
      final isCorrect = (_answers[i] == 1 && qs[i].answer) || (_answers[i] == 0 && !qs[i].answer);
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<TFQuestion>>(
        ...
        builder: (context, snap) {
          ...
          if (_answers.length != qs.length) _answers = List.filled(qs.length, -1);
            ...
            itemBuilder: (context, i) {
              if (i == qs.length) {
                return ElevatedButton(
                    onPressed: _answers.contains(-1) ? null : () => _submit(qs), child: Text('Submit Quiz'));
              }
              final q = qs[i];
              return Card(
                child: ListTile(
                  title: Text('Q${i + 1}: ${q.text}'),
                  subtitle: _submitted ? Text(_correctness[i] ? 'Correct' : 'Incorrect') : null,
                  trailing: Row(
                    ...
                    children: [
                      ChoiceChip(
                          ...
                          onSelected: _submitted ? null : (sel) => setState(() => _answers[i] = sel ? 1 : -1)),
                      SizedBox(width: 8),
                      ChoiceChip(
                          label: Text('False'),
                          selected: _answers[i] == 0,
                          onSelected: _submitted ? null : (sel) => setState(() => _answers[i] = sel ? 0 : -1)),
                    ],
                  ),
                ),
              );
